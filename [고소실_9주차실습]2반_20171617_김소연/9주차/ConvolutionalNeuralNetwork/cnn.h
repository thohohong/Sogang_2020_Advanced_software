				...
				for (int x = 0; x < size; x++) {
					for (int ky = 0; ky < conv->k_size; ky++) {														//이때 한 점은 kernel_size * kernel_size 번의 덧셈을 통해 계산
						for (int kx = 0; kx < conv->k_size; kx++) {
							...
							////////////////////////////////////////////////////////////////////////////////
							/* ADD YOUR CODE HERE */
							float* cur_input = input2D + x + conv->k_size / 2 + (y + 1) * input_size; //x, y에 대응하는 input의 좌표
							cur_input -= conv->k_size / 2 + input_size; //kernel의 0, 0에 대응하는 input의 좌표
							cur_input += kx + ky * input_size; //kx, ky에 대응하는 input의 좌표
							
							tmp[x + y * conv->f_size] += kernel2D[kx + ky * conv->k_size] * *cur_input;
						}
					}
				}
			}
