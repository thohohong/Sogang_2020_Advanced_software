...
double _f1(double** data, double sampling_num, double x, double u) {
    double result;
    double h;
...
double Bisection(double** data, double sampling_num, double a, double b, double u) {
    ...
    /* f(a) < 0, f(b) > 0 */
    if (_f1(data, sampling_num, b, u) > _f1(data, sampling_num, a, u)) {
        mid = a;
        a = b;
        b = mid;
    }
    ...
    for (int i = 0;; i++) {
        ...
        value = _f1(data, sampling_num, mid, u);
        ...
        if (_f1(data, sampling_num, a, u) * _f1(data, sampling_num, mid, u) < 0) {
            b = mid;
        }
        else {
            a = mid;
